...
1. **End with a completion marker**: `print MARK_<SLUG>_OK when done. go.`
2. **Include F1/F2/F3 final actions** for missions that commit code:
   - F1: `git status --short` — verify clean working tree
   - F2: `git log --oneline -5` — verify commits landed
   - F3: `printf '%s\n' 'MARK_<SLUG>_OK'` — pty-visible completion signal
...
print MARK_<SLUG>_OK when done. go.
```
...
F1: git status --short
F2: git log --oneline -3
F3: printf '%s\n' 'MARK_WORKER_[A/B/C]_OK'

print MARK_WORKER_[A/B/C]_OK when done. go.
```
