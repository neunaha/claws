...
| Rule | Requirement |
|---|---|
| Register | `claws_hello` with `waveId` + `subWorkerRole` within 60 s |
| Boot event | Publish `wave.<waveId>.<role>.boot` immediately after hello |
| Heartbeat | Publish `worker.*.heartbeat` every 20 s — server violation at 25 s |
| Phase events | Publish `worker.*.phase` on every lifecycle transition |
| Error events | Publish `worker.*.event` kind=ERROR for any blocking failure |
...
| Complete event | Publish `wave.<waveId>.<role>.complete` as final act |
| Close terminal | Close your own terminal after publishing complete |
...
```
1. claws_hello({ role: 'worker', peerName: '<waveId>-<role>', waveId, subWorkerRole: '<role>' })
2. Publish wave.<waveId>.<role>.boot
3. Start heartbeat loop (every 20 s)
4. Begin role-specific work
5. Publish phase events as work progresses
6. Publish complete event
7. Stop heartbeat loop
8. Close terminal
```
