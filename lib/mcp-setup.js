 ...
 * from text before passing to JSON.parse. Handles the common case in .mcp.json files
 * edited by VS Code's JSON language server. Matches install.sh's json-safe.mjs approach.
 * Note: does not handle comment sequences inside string values (edge case not seen in practice).
 * @param {string} text
 * @returns {string}
 */
