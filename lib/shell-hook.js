 ...
 * and sources scripts/shell-hook.ps1 which guards itself with CLAWS_WRAPPED=1
 * so the banner only appears inside Claws-wrapped terminals.
 * @param {string} installDir  - path to the claws repo (contains scripts/shell-hook.ps1)
 * @param {boolean} [dryRun]
 */
function _injectPowershellHook(installDir, dryRun) {
  const profilePath = getDefaultShellRcFile({ platform: 'win32' });
