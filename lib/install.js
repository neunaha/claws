...
function run(opts = {}) {
  ...
  // ── Phase 5: install commands + Bug 1 sweep ───────────────────────────────
  _step('Install commands (Bug 1 sweep)');
  installCommands(HOME, dryRun);
  _ok('Commands installed');

  // ── Phase 6: install skills + Bug 2 sweep ────────────────────────────────
...
function _injectClaudeMd(projectRoot, dryRun) {
  ...
  if (fs.existsSync(injectProject)) {
    const r = spawnSync(process.execPath, [injectProject, projectRoot, ...extraArgs], {
      cwd: REPO_ROOT, stdio: 'inherit', encoding: 'utf8',
    });
    if (r.status !== 0 && !dryRun) _warn('inject-claude-md.js failed — CLAUDE.md may be stale');
  }
