...
function _installExtension(codeCli, dryRun) {
  ...
  // Package VSIX
  _info('vsce package...');
  spawnSync('npx', ['@vscode/vsce', 'package', '--no-dependencies', '-o', vsixPath],
    { cwd: extDir, stdio: 'inherit', shell: isWin });
  ...
  const install = spawnSync(codeCli, ['--install-extension', vsixPath, '--force'],
  ...
  // Verify installation
  const listR = spawnSync(codeCli, ['--list-extensions'], { encoding: 'utf8', shell: isWin });
