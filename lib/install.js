...
function run(opts = {}) {
  ...
  // Install when: (a) CLAWS_INSTALL_DEV_HOOKS=1, OR (b) this IS the Claws source tree.
  // Detection: presence of scripts/install.sh + extension/src/ inside REPO_ROOT.
  const isDevTree = fs.existsSync(path.join(REPO_ROOT, 'scripts', 'install.sh')) &&
                    fs.existsSync(path.join(REPO_ROOT, 'extension', 'src'));
...
function _cleanStaleExtensionDirs(label, version) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    while (Date.now() < end) { /* spin-wait 200ms */ }
  }
