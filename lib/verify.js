...
function verify(projectRoot) {
  ...
  // Check project-local commands first; fall back to global when projectRoot not given.
  const cmdPath = projectRoot
    ? path.join(projectRoot, '.claude', 'commands', 'claws.md')
    : path.join(CLAUDE_DIR, 'commands', 'claws.md');
