...
### Fixed
- **W7h-14 — atomic hooks/ copy in _prepareCLawsBin** — Replace `fs.cpSync(hooksSrc, hooksDir)` with `_copyDirAtomic` (copy to tmp → move old aside → rename tmp → clean old), matching the M-09 pattern from `scripts/_helpers/atomic-file.mjs`. Avoids partial `.claws-bin/hooks/` state if the process is killed mid-copy.
