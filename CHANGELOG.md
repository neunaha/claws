...
- **Bug 11 — `claws_dispatch_subworker` `_dswCwd` undeclared (Phase 1)** — `ReferenceError: _dswCwd is not defined` thrown inside the `setImmediate` boot block, silently caught by the surrounding `try/catch`, preventing Claude from launching in every dispatched sub-worker. Symptom: LEAD spawns sub-worker via `claws_dispatch_subworker`, terminal appears in `claws_list`, Claude never boots, terminal stays open forever, `claws_workers_wait` times out. Fix: add `const _dswCwd = typeof args.cwd === 'string' && args.cwd.length > 0 ? args.cwd : null;` before `setImmediate`, mirroring the explicit cwd-alias pattern in `runBlockingWorker` and the `claws_worker` fast-path. Phase 1 of 3 — minimal baseline fix; Phases 2–3 extract shared spawn-and-watch helpers to prevent this class of copy-paste regression. Root cause documented in `.local/plans/v0714/investigations/army-shell-autoclose.md` (Divergence 2).
