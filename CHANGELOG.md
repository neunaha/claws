...
### Changed
- **Wave 6X — bundle-native.mjs harvests node-pty npm prebuilds for all platforms** — Replaces the Wave 6A try-rebuild-or-prebuilt decision tree with a single step: copy `node_modules/node-pty/prebuilds/<plat>/` → `native/node-pty/prebuilds/<plat>/` for `darwin-arm64`, `darwin-x64`, `win32-x64`, `win32-arm64`, filtering `.pdb` files. Also copies `lib/`, `package.json`, `LICENSE`, `README.md`. Atomic staging (`.claws-new` / `.claws-old` rename) preserved. Drops `detectElectronVersion`, `detectTargetArch`, `runElectronRebuild`, `hasPython`, `hasCompiler`, `CLAWS_FORCE_REBUILD`, sha256 manifest verification. `--strict` flag retained for CI. `extension/src/extension.ts` diagnostics extended to report `native/node-pty/prebuilds/<platform>-<arch>/pty.node` as the primary canonical path.
