...
### Critical fixes (Wave AE-7)
- **AE-7 — event-driven submit confirmation with escalating retry**: The 15 s polling+nudge loop in `_sendAndSubmitMission` was the same fragile fixed-cadence pattern already rejected for boot detection. Replaced with a pure bus-event architecture: after the initial `\r` (strat 0), the function awaits `_waitForSubmitEvent` — a new helper that blocks up to `RETRY_INTERVAL_MS` (3 s) for one of three bus signals: `vehicle.<termId>.content` (pty content changed) with a pty-log size check, `tool.+.invoked` from the worker's peer (strongest signal — Claude is calling MCP tools), or lifecycle-end (`system.terminal.closed`/`system.worker.terminated`). On timeout the loop escalates to the next strategy in `SUBMIT_STRATEGIES = ['\r', '\r\n', '\x1b[201~\r', '\n\r', '\x1b\r']` and loops. `SUBMIT_CEILING_MS` (60 s) is a true-hang safety net, not a budget — on Mac the `tool.invoked` event arrives in ~1 s so strat 0 resolves without ever escalating. Zero `process.platform` branches — the same strategies run on darwin/linux/win32. `_pconnHandleData` extended to dispatch `vehicle.<termId>.content` → `_submitWaiters` and `tool.+.invoked` → `_submitWaiters` (via new `_peerIdToCorrId` map, populated on `system.peer.connected`). `_pconnEnsureRegistered` subscribes lazily to `vehicle.+.content` and `tool.+.invoked` with reconnect-safe flags. `_sendAndSubmitMission` signature gains `corrId` parameter (updated at all three call sites: `runBlockingWorker`, `claws_worker`, `claws_dispatch_subworker`). Tests: `paste-gate.test.js` assertion 10 replaced with two AE-7 invariants (SUBMIT_STRATEGIES has `\r` first + ≥ 3 entries; `_sendAndSubmitMission` uses `_waitForSubmitEvent` and has no polling readLog loop); `mac-event-driven-boot.test.js` gains I6 section (3 new checks: `_waitForSubmitEvent` defined, SUBMIT_STRATEGIES shape, no `●`/`⏺` regex in `_sendAndSubmitMission`); `worker-fixes-v079.test.js` W8k deadline assertion broadened to accept either `15000` or `SUBMIT_CEILING_MS`. All green: 20/20 + 15/15 + 42/42.

### Critical fixes (Wave AE-6.b)
- **AE-6.b — replace time-based boot race with event-driven wait**: `_waitForWorkerReady` in `mcp_server.js` previously used an 8 s (darwin/linux) or 15 s (win32) timeout as a boot budget. On the Azure Windows VM, Claude's cold-cache boot occasionally hit exactly the boundary — term 27 (corr `098215da`) received `boot_failed @ 12:29:05.289Z` followed 5 ms later by `peer.connected @ 12:29:05.294Z`. Bumping the number was wrong architecture. Fix: `_waitForWorkerReady` now uses a 120 s **safety ceiling** (not a budget) applied identically on all three platforms. No `process.platform` branch in the boot path. Additionally, the function now registers early-abort listeners for `system.terminal.closed` and `system.worker.terminated` keyed by `correlation_id` — if the worker's terminal is killed before the ready event arrives, the promise rejects immediately rather than waiting for the ceiling. `_pconnHandleData` extended to dispatch these cancel topics to `_workerReadyWaiters`. `boot_wait_ms` removed from `runBlockingWorker` DEFAULTS; all three call sites (slow path, fast path, dispatch_subworker) pass `args.boot_wait_ms` directly (undefined → 120 s ceiling). `_gatePasteOnClaudeClaim` drops its `|| 8000` fallback. Phase A (reverts AE-4's `\n` submit keystroke, confirmed in `_sendAndSubmitMission`): `_submitKey = '\r'` with no `process.platform` branch — already landed prior to this wave; locked in by paste-gate assertion 10. Three test suites updated: `paste-gate.test.js` assertion 9 changed to negative assertion (no sub-10000 hardcoded default); `mac-event-driven-boot.test.js` I5.a changed to assert `ceilingMs >= 60000`; `worker-fixes-v079.test.js` W8ac-2 assertion updated + W8k-1 broadened to match `_submitKey` variable pattern. All green: 19/19 + 12/12 + 42/42.

### Critical fixes (Wave AE-4)
- **AE-4 — Windows ConPTY mission-submit: platform-branched submit keystroke**: On Windows VM, workers spawned cleanly and claude TUI booted (`system.peer.connected` fired, AD-1 gate released, mission paste rendered in input), but the trailing `\r` did not submit — the 15 s/5-nudge CR loop fired all nudges without effect and the user had to manually nudge. Root cause: ConPTY on Windows does not reliably trigger Ink's Enter handler when `\r` (0x0D) is written programmatically via `pty.write()` after a bracketed-paste sequence; manual keyboard Enter (which flows through xterm.js's native key path) does trigger it. Fix: `_sendAndSubmitMission` in `mcp_server.js` now resolves the submit keystroke via `_submitKey = process.platform === 'win32' ? '\n' : '\r'`; both the initial submit send and all 5 nudges use `_submitKey`. Darwin/Linux path unchanged. Probe methodology documented in `.local/audits/windows-mission-submit-probe.md`. `paste-gate.test.js` assertion #19 locks in the `process.platform === 'win32'` branch via static analysis. 19/19 PASS.

### Tests (Wave AE-5)
- **AE-5 — lock in Mac event-driven boot UX invariants**: Static-analysis test `extension/test/mac-event-driven-boot.test.js` encodes the full event-driven boot path as a non-negotiable contract (12 checks). Invariant groups: I1 — `mcp_server.js` `main()` eager-hello block is guarded on `process.env.CLAWS_TERMINAL_CORR_ID`, calls `_pconnEnsureRegistered` inside `setImmediate`, and does NOT call `fs.existsSync` (AE-1.1 win32 named-pipe regression guard; checking only code, not comments, via `//`-line stripping); I2 — `server.ts` `hasRootOrchestrator()` excludes peers with both `!p.waveId` AND `!p.correlationId` (AE-1.2), and the hello-handler root-rejection gate exempts orchestrators carrying a `correlation_id` (AE-1); I3 — `_gatePasteOnClaudeClaim` body is free of `claudeMarkers`, `shellErrorMarkers`, and `pty_tail` (regex/pty-log fallback remains removed), and contains `cause: 'event_driven_boot_timeout'` as the clean abort signal; I4 — exactly ONE `await sleep(` call in `_gatePasteOnClaudeClaim` and it is `await sleep(200)` (200ms safety buffer only — no creeping sleeps); I5 — `_waitForWorkerReady` default `timeoutMs` for `type:'claude'` is `8000` (flags any silent budget increase). Wired as `test:mac-event-driven-boot` in `extension/package.json` and appended to the `test` chain immediately after `test:paste-gate`. Tri-platform: pure `fs.readFileSync` + regex — no fork, no child_process, no platform paths; runs identically on darwin/linux/win32. 12/12 PASS. `paste-gate.test.js` still 18/18 (no regression).

### Infrastructure (Wave AE-3)
- **AE-3 — installer hygiene: kill stale worker processes before install**: All three installer entry points (`scripts/install.sh`, `scripts/install.ps1`, `lib/install.js`) now run a pre-install cleanup phase that kills orphaned `mcp_server.js` and `spawn-helper` processes from prior worker sessions before touching `.claws-bin/`. Root cause addressed: worker terminals that exit without cleanly deregistering leave their child `mcp_server.js` alive; on restart those processes hold the root-orchestrator slot in the peer registry and block new outer-orchestrator hellos. Cleanup is safe: it only kills processes whose command line references the project's `.claws-bin/mcp_server.js` path AND whose environment contains `CLAWS_TERMINAL_CORR_ID` (= worker children spawned by the extension). The outer orchestrator running the installer never has that env var set, so it is never killed. On Mac/Linux, `mcp_server.js` is filtered by `pgrep -af` + full-path match + `CLAWS_TERMINAL_CORR_ID` env check (`ps -E -p <pid>` on macOS; `/proc/<pid>/environ` on Linux). Stale `spawn-helper` processes (orphaned node-pty wrappers) receive the same env check. On Windows, `Get-CimInstance Win32_Process` filters `node.exe` by `CommandLine` containing the target path, then `Stop-Process` kills them (env introspection not required on Windows since the path match is unique enough). All three installers also clear a stale `.claws/sidecar.pid` if the recorded PID is no longer alive. Cleanup is idempotent — a fresh install with no orphans produces "killed 0 stale processes, cleared 0 stale pid files". Static-analysis regression test: `extension/test/install-cleanup.test.js` (10 checks) confirms all three entry points contain the expected APIs. Mac smoke: `node lib/install.js` in dryRun mode exits 0 with no collateral kills on this machine.

### Critical fixes (Wave AE-1.2)
- **AE-1.2 — `hasRootOrchestrator()` must exclude correlation_id-carrying nested orchestrators**: Symmetric counterpart to AE-1's rejection-gate exemption. AE-1 made the hello-rejection gate (`server.ts:1241`) accept orchestrators with a `correlation_id` (so worker mcp children can register), but `hasRootOrchestrator()` at line 983 still counted them as roots. Effect on Windows VM: when worker children eager-hellos finished BEFORE the outer orchestrator's own first hello, they occupied the root slot, and every subsequent root hello (from the user's outer Claude Code mcp_server.js) was rejected as "root orchestrator already registered". Outer never registered, never subscribed to `system.peer.connected`, gate never released, mission paste never sent. Fix: `hasRootOrchestrator()` now requires `!p.correlationId` in addition to `!p.waveId` — nested orchestrators coexist with the user's root, they do not occupy the slot. Pure JS, no platform branches.

### Critical fixes (Wave AE-1.1)
- **AE-1.1 — drop fs.existsSync guard around eager-hello (win32 named-pipe fix)**: AE-1's eager-hello block was guarded by `if (_aeSock && fs.existsSync(_aeSock))`. On Windows the Claws bus is a named pipe (`\\.\pipe\claws-<hash>`) which has no filesystem presence — `fs.existsSync` returns `false`, so the eager hello was silently skipped on Windows. Workers booted, no `system.peer.connected` fired, AD-1 gate timed out at 8 s, `boot_failed` events fired, terminals closed before Claude finished booting. Fix: drop the `existsSync` check entirely. `_pconnEnsureRegistered` already handles connect failures via the circuit breaker — one logged warning, no crash. The eager hello now runs uniformly on darwin/linux/win32. Pure JS, single platform-agnostic code path. Confirmed via `node -e 'console.log(require("fs").existsSync("\\\\.\\\\pipe\\\\claws-..."))'` returning `false` on Windows Server 2022 VM.

### Critical fixes (Wave AE-1)
- **AE-1 — event-driven boot via eager hello**: Architectural fix for the `system.peer.connected` blackout (audit: `.local/audits/peer-connected-blackout-root-cause.md`). Root cause: `mcp_server.js` hellos lazily — only when the first MCP tool call demands bus access. A freshly-booted Claude TUI at the empty prompt invokes no tool, so its child `mcp_server.js` never hellos, so `system.peer.connected{correlation_id}` never fires, so the AD-1 gate always times out for cold Claude TUI workers. Env propagation through claude was verified correct (probe pid 80478 had `CLAWS_TERMINAL_CORR_ID`); the bug was hello timing, not env scrubbing. Three coordinated changes — pure JS, no platform branches: (1) `mcp_server.js` `main()`: eager hello at startup when `CLAWS_TERMINAL_CORR_ID` is set — indicates we are a worker's child; `setImmediate` so stdin reading isn't blocked. (2) `extension/src/server.ts` hello-handler: exempt orchestrators carrying a `correlation_id` from the rootOrchestrator gate — they are nested-by-terminal, not competing roots. (3) `mcp_server.js _gatePasteOnClaudeClaim`: drop the regex / pty-log fallback per user directive ("event-driven architecture only, more robust"); timeout now → clean `boot_failed` with cause `event_driven_boot_timeout`. `paste-gate.test.js` updated: drops the fallback-regex assertions, adds AE-1 invariants (no `claudeMarkers`/`shellErrorMarkers`/`pty_tail` in helper, eager-hello block present in `main()`, `setImmediate` guarded on env). 18/18 PASS. Existing tests still green: `worker-fixes-v079` 42/42.
...
### Critical fixes (Wave AC-1.1)
- **W8ac-1.1 — spawn-helper executable bit: bundle-native chmod + runtime diagnostic**: Mac regression surfaced after AC-1 VSIX rebuild: every wrapped pty fell back to pipe-mode with `posix_spawnp failed`. Root cause: `spawn-helper` binaries in `extension/native/node-pty/prebuilds/darwin-{arm64,x64}/` were stored at mode 0644; `node-pty` uses `posix_spawnp(3)` to exec `spawn-helper` before forking the user shell, so EACCES silently triggered the pipe-mode fallback. (1) `bundle-native.mjs`: after `copyTree()` for each platform prebuild, `chmodSync(spawnHelperDest, 0o755)` guarded by `existsSync` — fires for darwin-*/linux-*, silently skips win32 (no spawn-helper). Logged for diagnostic visibility. Every future VSIX build ships spawn-helper as +x. (2) `claws-pty.ts`: defensive runtime check in `open()` before the spawn attempt — on darwin/linux, `statSync` spawn-helper and warn loudly if `mode & 0o111 === 0`, citing `posix_spawnp` so future debugging maps symptom to cause. Does not auto-chmod at runtime (self-healing would mask the upstream packaging bug). (3) Source tree: `chmod 0755` + `git update-index --chmod=+x` on both darwin spawn-helpers so the repository is the correct reference. (4) `worker-fixes-v079.test.js`: 2 new checks → 42 total (belt-and-suspenders: bundle-native.mjs contains `chmodSync`/`spawn-helper`/`0o755`; source-tree binaries are all +x on darwin/linux). Tri-platform: darwin + linux affected (posix_spawnp path); win32 unaffected (winpty/conpty, no spawn-helper); all branches covered explicitly.

### Chore
- **Bake tri-platform convention into CLAUDE.md + drop orphan W8aa test**: Adds the tri-platform-first rule as a named convention in CLAUDE.md (every code path designed for darwin + linux + win32 from the design phase; sister-bug rule). Removes the W8aa-1 check from `worker-fixes-v079.test.js` — that entry was an orphan from an aborted wave (Wave AA, superseded by Wave AC). The assertion (`boot_wait_ms=25000`) targeted a fast-path default that is invalid under the AC event-driven design. No behavioural code changed. Suite remains at 34 PASS.

### Infrastructure (Wave AC-2)
- **W8ac-2 — event-driven boot via correlation_id**: Replaces TUI-byte-pattern boot detection (polling for ❯ + `cost:$` stable×3) with event-driven subscription to `system.peer.connected` (Claude workers) and `system.terminal.ready` (shell workers) keyed by `correlation_id`. Three sites rewired: `runBlockingWorker` slow path, `claws_worker` fast path, `claws_dispatch_subworker`. New `_waitForWorkerReady(sock, corrId, opts)` helper resolves on the matching bus event or falls back after `timeoutMs` with a log (no abort — preserves best-effort behavior on version skew). 5s post-boot settle dropped; 200ms safety buffer retained (blueprint §9). `correlation_id` now flows end-to-end: generated before `claws_create`, forwarded in the create payload, injected as `CLAWS_TERMINAL_CORR_ID` env by the backend (AC-1), read by child `mcp_server.js` in self-hello. `fast-path boot_wait_ms` default aligned to 8000 (was 25000 — now event-driven, not a polling budget). Tri-platform: no `process.platform` branches; bus subscription is uniform. 6 new regression checks in `worker-fixes-v079.test.js` (40 total). Depends on Wave AC-1 (5ebea6f).
...
### Critical fixes (Wave R)
- **W8r-1 — Windows runtime: sidecar eventsLog, fileExec, wrapShellCommand**: Three `mcp_server.js` bugs that only surface on Windows. (1) W8Q3-1: `_spawnAndVerifySidecar` computed `eventsLogFilePath` via `path.dirname(path.resolve(socketPath))` — on Windows `socketPath` is a named pipe (`\\.\pipe\...`) so dirname resolves to the pipe namespace, causing `fs.createWriteStream` to fail with EACCES/ENOENT and hard-blocking all spawn-class tools. Fixed by replacing the two-line derivation with a single `_eventsLogPath(socketPath)` call (the correct helper was already defined at line 1834). (2) W8Q3-2: `fileExec` sent a POSIX `{ cmd; } > outPath 2>&1; echo $? > donePath` wrapper — PowerShell does not understand `{ }` subshell grouping or `echo $?`. Added win32 branch: `& { <cmd> } *> '<outPath>' ; $LASTEXITCODE | Out-File -FilePath '<donePath>' -Encoding ascii`. (3) W8Q3-4: `wrapShellCommand` appended POSIX `printf` for the `[CLAWS_PUB]` bus event and `__CLAWS_DONE__` marker — `printf` is absent in PowerShell, so shell workers never signalled completion. Added win32 branch using `Write-Output`. Non-win32 branches untouched. Audit source: `.local/audits/w8q3-paranoid-windows-audit.md`.

### Tests (Wave Y)
- **W8y-1 — regression tests for Waves R + V in worker-fixes-v079**: Added 4 static-analysis checks to `extension/test/worker-fixes-v079.test.js` locking in the Windows runtime fixes from Wave R (611108a — W8Q3-1 sidecar eventsLog path, W8Q3-2 fileExec PowerShell branch, W8Q3-4 wrapShellCommand Write-Output branch) and Wave V (a1b7387 — W8Q3-3 win32 pid-file sidecar dedup + `process.kill` liveness probe). Suite grows from 25 to 29 checks, all green.
...
- **W8j-3 — monitor_arm_command uses forward-slash path on win32**: `STREAM_EVENTS_JS` resolves with backslashes on Windows (`path.join` produces `C:\...\stream-events.js`). When embedded verbatim in `monitor_arm_command` strings, the single backslashes were eaten by shell quoting at the Monitor tool receiver (`\s`, `\t`, `\.` silently dropped), making the path invalid. Added `STREAM_EVENTS_JS_FOR_CMD` constant that replaces backslashes with forward slashes on `win32` (Node accepts both; fs.existsSync candidates list is unaffected). Replaced all five Monitor command template-string embedding sites.
...
- **W8j-2 — events_log_path is a filesystem path, not a pipe name on Windows**: On Windows `sock` is a named pipe (`\\.\pipe\claws-XXXXXXXX`), so `path.dirname(path.resolve(sock))` resolved to `\\.\pipe` and `events_log_path` was returned as `\\.\pipe\events.log` — neither a valid pipe nor a file. Added `_eventsLogPath(sock)` helper that on `win32` derives the workspace root via `CLAWS_WORKSPACE_ROOT` env var or `_findWorkspaceRoot()` walk and returns `<root>/.claws/events.log`; on other platforms the existing `path.dirname(path.resolve(sock))` logic is unchanged. All four spawn-response call sites updated.
...
- **W7-5 — claws_fleet handler trace logging for Windows debug**: Added `log()` checkpoints in the `claws_fleet` handler at enter, per-worker spawn-begin/spawn-ok/spawn-error, dispatched, and exit. Enables triangulation of Windows-specific failures (long paths, bracketed-paste edge cases) by showing exactly where in the async fleet pipeline a failure occurs.
...
### Changed
- **Wave 6X — bundle-native.mjs harvests node-pty npm prebuilds for all platforms** (`refactor` + `chore` commits) — Replaces the Wave 6A try-rebuild-or-prebuilt decision tree with a single step: copy `node_modules/node-pty/prebuilds/<plat>/` → `native/node-pty/prebuilds/<plat>/` for `darwin-arm64`, `darwin-x64`, `win32-x64`, `win32-arm64`, filtering `.pdb` files. Also copies `lib/`, `package.json`, `LICENSE`, `README.md`. Atomic staging (`.claws-new` / `.claws-old` rename) preserved. Drops `detectElectronVersion`, `detectTargetArch`, `runElectronRebuild`, `hasPython`, `hasCompiler`, `CLAWS_FORCE_REBUILD`, sha256 manifest verification. `--strict` flag retained for CI. `extension/src/extension.ts` diagnostics extended to report `native/node-pty/prebuilds/<platform>-<arch>/pty.node` as the primary canonical path.

### Removed
- **Wave 6X — drop Wave 6A prebuilt/ directory and bundle-native-prebuilt regression test** — `extension/native/node-pty/prebuilt/` (manifest.json + darwin-arm64/pty.node) deleted. `bundle-native-prebuilt.test.js` deleted. Tests for removed bundle-native.mjs exports deleted: `bundle-native-arch.test.js`, `bundle-native-spawn.test.js`, `bundle-native-timeout.test.js`, `bundle-native-editor-detect.test.js`, `bundle-native-multiarch.test.js`. All removed tests deregistered from `package.json` scripts and `npm test` chain.
...
- **v0.8 Wave 6A — regression test: prebuilt sha256 stability** — `extension/test/bundle-native-prebuilt.test.js` (7 checks): manifest well-formed, darwin-arm64 entry present with sha256/built_at/host, `prebuiltPath()` returns correct path, `verifySha256()` passes for known-good file, `verifySha256()` returns false for wrong hash, build output sha256 matches manifest (Mac regression contract). Added as `test:bundle-native-prebuilt` in `package.json` and wired into the main `npm test` chain.
- **v0.8 Wave 6A — ship darwin-arm64 prebuilt pty.node** — `extension/native/node-pty/prebuilt/darwin-arm64/pty.node` committed (85 592 bytes, sha256 `64249103…`). Built on macOS 15.6.1 / Apple clang 17.0.0 targeting Electron 39.8.8 arm64 using N-API 4 (ABI-stable across VS Code Electron versions). `prebuilt/manifest.json` updated with `darwin-arm64` entry. After this commit, `npm run build` on any Mac Apple Silicon takes the prebuilt path and skips `@electron/rebuild` entirely.

### Fixed
- **W7h-30C — re-add explicit PreToolUse MCP spawn-class matchers (BUG-28 parity)**: `scripts/inject-settings-hooks.js` HOOK_ENTRIES now includes four PreToolUse entries with explicit matchers for `mcp__claws__claws_create`, `mcp__claws__claws_worker`, `mcp__claws__claws_fleet`, and `mcp__claws__claws_dispatch_subworker`. These were removed in v0.7.13 (T4) in favor of server-side gates; re-added as belt-and-suspenders for Claude Code builds where `*` is not propagated to MCP tool calls (Windows and some older builds). The server-side gate in `mcp_server.js` remains the primary enforcer. Test `inject-settings-dedup` updated: PreToolUse expected count 2 → 6.
- **W7h-30B — duplicate hooks/hooks/ path in global hook registration**: `installGlobalHooks()` in `lib/install.js` was returning `~/.claude/claws/hooks` as `CLAWS_BIN`, which `inject-settings-hooks.js` then passed to `path.join(CLAWS_BIN, 'hooks', scriptName)`, producing `~/.claude/claws/hooks/hooks/<script>.js`. Fixed by returning the parent directory `~/.claude/claws` so the inject script builds the correct `~/.claude/claws/hooks/<script>.js` path. The actual hook files are still copied into `~/.claude/claws/hooks/` as before.
- **W7h-30A — win32 direct node invocation in hookCmd non-canonical path**: On Windows there is no `sh` in PATH, so the `sh -c` fault-tolerance wrapper in the non-canonical `hookCmd` fallback in `scripts/inject-settings-hooks.js` produced silent "sh not found" errors, silently breaking all Claws lifecycle hooks. When `process.platform === 'win32'`, emit direct `node "<path>"` instead, matching the canonical-install fast path. The `sh -c` wrapper is unchanged on Unix. Matches `install.sh` behaviour where Unix sh-wrappers are emitted only on non-Windows.
- **W7h-31 — dev-hooks install for contributor environments** (fix: isDevTree checks `projectRoot` instead of `REPO_ROOT` so non-repo installs don't trigger dev-hooks; parity test now passes `CLAWS_SKIP_EXTENSION_COPY=1` to match bash installer): `run()` in `lib/install.js` now runs Phase 6b: when `CLAWS_INSTALL_DEV_HOOKS=1` or the install target is the Claws source tree (`scripts/install.sh` + `extension/src/` present in REPO_ROOT), `_installDevHooks()` copies `scripts/dev-hooks/*.js` to `<project>/.claws-bin/dev-hooks/` and runs `scripts/inject-dev-hooks.js`. Default user installs skip this entirely. Matches `install.sh`:1301-1332.
- **W7h-28 + W7h-29 — verify.js MCP handshake + hook registration**: `verify()` in `lib/verify.js` now runs two additional checks after file-existence verification. W7h-28: spawns `mcp_server.js` with a JSON-RPC `initialize` request and expects a response containing `"claws"` within 3s (matching `install.sh`:1540-1557). W7h-29: parses `~/.claude/settings.json` and asserts `PreToolUse` and `PostToolUse` spawn-class hooks contain a `mcp__claws__claws_worker` matcher (matching `install.sh`:1508-1528).
- **W7h-27 — .mcp.json post-write JSON validation**: `writeMcpJson` in `lib/mcp-setup.js` now re-reads and `JSON.parse`s the file after `fs.renameSync` to catch any FS-level corruption, warning on parse failure. Matches `install.sh`:1090-1093.
- **W7h-25 + W7h-26 — stale extension dir cleanup + sudo fallback**: `_installExtension` in `lib/install.js` now retries a failed `--install-extension` with `sudo` on Linux/macOS (matching `install.sh`:714-719). After a successful install, `_cleanStaleExtensionDirs()` polls up to 1s for VS Code to extract the new VSIX, then removes any `neunaha.claws-<old-version>` dirs from the editor's extensions directory (matching `install.sh`:740-760). Works across all 4 editors (`code`, `code-insiders`, `cursor`, `windsurf`).
- **W7h-24 — CLAWS_GLOBAL_MCP + CLAWS_GLOBAL_CONFIG env support**: `CLAWS_GLOBAL_MCP=1` registers the claws MCP server entry in `~/.claude/settings.json` `mcpServers` (matching `install.sh`:1150-1163). `CLAWS_GLOBAL_CONFIG=1` installs commands/skills/rules into `~/.claude/` in addition to the project-local `.claude/` (matching `install.sh`:1297-1299). Both are opt-in with no effect when unset.
- **W7h-20 + W7h-21 — Electron ABI drift detection + VSIX size check**: `_installExtension` in `lib/install.js` now calls `_checkElectronAbiDrift()` before packaging, comparing `extension/native/.metadata.json` `electronVersion` against the installed editor's version (macOS plist / Linux electron binary). Warns on mismatch with rebuild instructions. Respects `CLAWS_ELECTRON_VERSION` env override. Also checks VSIX file size after packaging and warns when <50 KB (native binary likely missing). Matches `install.sh`:403-454 and :697-703.
- **W7h-13 — extension/ artifacts copied to .claws-bin/extension/**: `_prepareCLawsBin` now calls `_copyExtensionArtifacts()` which copies `dist/`, `native/`, `package.json`, `package-lock.json`, `README.md`, `CHANGELOG.md`, `icon.png`, `.vscodeignore` from `extension/` into `<project>/.claws-bin/extension/` for visibility. Matches `install.sh` lines 968-983. Opt out with `CLAWS_SKIP_EXTENSION_COPY=1`.
- **W7h-14 — atomic hooks/ copy in _prepareCLawsBin**
- **Wave C — fix JSDoc comment syntax errors in install.js + mcp-setup.js** — Block JSDoc comments containing `*/` inside the comment body caused Node.js to prematurely end the block comment, producing `SyntaxError: Unexpected token ','` on module load. Replaced offending multi-line JSDoc blocks with inline `//` comments. — Replace `fs.cpSync(hooksSrc, hooksDir)` with `_copyDirAtomic` (copy to tmp → move old aside → rename tmp → clean old), matching the M-09 pattern from `scripts/_helpers/atomic-file.mjs`. Avoids partial `.claws-bin/hooks/` state if the process is killed mid-copy.
- **W7h-12 — .mcp.json merge tolerates JSONC comments** — `writeMcpJson` and `removeMcpEntry` in `lib/mcp-setup.js` now strip JSONC (line comments, block comments, trailing commas) via inline `_stripJsonc` before `JSON.parse`. Projects whose `.mcp.json` contains VS Code JSONC comments no longer silently skip claws MCP registration. Matches `install.sh` json-safe.mjs approach.
- **W7h-11 — preflight adds npm ≥ 7, git ≥ 2, C++ toolchain, python3, disk space checks** — `lib/preflight.js` `run()` now returns `{ failures, warnings }` (previously bare array). `lib/install.js` prints warnings non-fatally before continuing. New WARN-not-block checks: npm ≥ 7, git ≥ 2, C++ toolchain (Xcode CLT on darwin; g++/make on linux; cl.exe/msbuild on win32), python3, disk space ≥ 512 MB free in HOME. Matches `install.sh` preflight block.
- **W7h-9 + W7h-10 — .gitignore + .vscode/extensions.json updates** — `_prepareCLawsBin` now calls `_updateGitignore` (appends `.claws/`, `.mcp.json`, `.claws-bin/` to project `.gitignore` if absent; matches `install.sh` lines 1094-1106) and `_updateVscodeExtensions` (merges `neunaha.claws` into `.vscode/extensions.json` `recommendations`; JSONC-tolerant; matches `install.sh` lines 1113-1144). Both are idempotent and use atomic tmp+rename writes.
- **W7h-17 + W7h-18 — shell-hook fish conf.d + nushell support** — `getDefaultShellRcFile()` in `lib/platform.js` corrected: fish now returns `~/.config/fish/conf.d/claws.fish` (auto-sourced by fish) instead of `config.fish`. `_injectFishHook()` writes a minimal conf.d file (set CLAWS_DIR + conditional source of `shell-hook.fish`) when `~/.config/fish/` exists. `_injectNushellHook()` appends `$env.CLAWS_DIR` + `$env.CLAWS_SOCKET` to `~/.config/nushell/env.nu` (falls back to `config.nu`) when present; idempotent (skips if CLAWS_DIR already in file). Both called from `injectShellHook`. Matches `install.sh` lines 1435-1466.
- **W7h-15 + W7h-16 — shell-hook backup + syntax validation** — `_injectIntoFile` now backs up each rc file to `<rc>.claws-bak.<ISO-timestamp>` before modification (matches `install.sh` lines 1360-1369). After injection, runs `zsh -n` (for `.zshrc`) or `bash -n` (for `.bashrc`/`.bash_profile`) and warns with backup path on failure (matches `install.sh` lines 1423-1432). New `_getValidatorShell()` exported for tests.
- **W7h-8 — shell-hook covers ~/.zshrc + ~/.bashrc + ~/.bash_profile** — `injectShellHook` now injects into all standard rc files instead of a single detected shell file. macOS: `~/.zshrc`, `~/.bashrc`, `~/.bash_profile`. Linux: `~/.zshrc`, `~/.bashrc`. Matches `install.sh` lines 1419-1433 multi-file injection. New `_getStandardRcFiles()` exported for tests.
- **W7h-7 — VSIX flags match install.sh canonical set** — `_installExtension` in `lib/install.js` now passes `--skip-license --no-git-tag-version --no-update-package-json` to `npx @vscode/vsce package` (previously missing). VSIX output path moved from `<REPO_ROOT>/claws-code-<ver>.vsix` to `os.tmpdir()` to avoid leaving artifacts in the git working tree. VSIX cleaned up after all editor installs complete.
- **W7h-6 — preflight demotes missing VS Code CLI to soft warning** — Removed the hard-fail `'VS Code CLI (code) not found'` check from `lib/preflight.js`. Missing editor CLI is now a soft warning emitted by `_installExtension` itself (already implemented: `editorClis.length === 0 → _warn + return`). Result: missing editor CLI → install completes phases 1-6 + 8, phase 7 warns and skips. Matches `install.sh` lines 205-227 warn-and-continue behavior.
- **W7h-4 — .mcp.json absolute path in args** — `writeMcpJson` in `lib/mcp-setup.js` now writes `path.join(projectRoot, '.claws-bin', 'mcp_server.js')` (absolute) instead of the relative `.claws-bin/mcp_server.js`. Matches `install.sh` line 1072. Parity harness normalizes both to relative before comparing, so no parity regression.
- **W7h-3 — multi-editor CLI discovery** — `lib/platform.js` adds `findAllEditorClis()` returning `[{label, cliPath}]` for any of `code`, `code-insiders`, `cursor`, `windsurf` found on the system (PATH lookup + macOS bundle paths + Linux /usr/bin + /snap). `findCodeCli()` kept as backward-compat shim. `_installExtension` in `lib/install.js` now accepts the full editor array, builds/packages the VSIX once, and installs into every found editor; succeeds if at least one succeeds.
- **W7h-5 — lib/verify.js checks project-local skills+rules (W7-6 residual)** — `verify()` and `status()` now check `<projectRoot>/.claude/skills/` and `<projectRoot>/.claude/rules/` instead of global `~/.claude/`. Matches where `installSkills`/`installRules` actually write (commit `89177a4`, W7-6). Removes the stale `CLAUDE_DIR` constant. Fixes stale path assertions in `test/installer.test.js` tests (a), (c), and (i) that checked `tmpHome/.claude/` instead of `tmpProject/.claude/`; all 13 installer tests now green.
- **W7h-2 — hook scripts installed to `~/.claude/claws/hooks/` (Option B)** — `_injectHooks()` now calls `installGlobalHooks()` to atomically copy `scripts/hooks/*.js` into `$HOME/.claude/claws/hooks/` before registering them in `~/.claude/settings.json`. Hooks no longer reference the project-local `.claws-bin/hooks/` path, which is gitignored and deleted on project moves. Adds 2 tests: (l) global hooks dir created with hook scripts; (m) settings.json still points at global path after project `.claws-bin/` deletion.
- **W7h-1 — lib/shell-hook.js unifies block marker with install.sh canonical format** — Changed from `# >>> claws-code shell hook >>>…<<<` fenced block to install.sh's two-line pattern: `# CLAWS terminal hook` marker + `source` line. `_removePriorBlock` now strips both formats so bash→npm transitions don't accumulate duplicate blocks in `~/.zshrc`/`~/.bashrc`. Adds `test/shell-hook.test.js` (7 checks).
- **W7-6 — lib/install.js phase 4 installs skills+rules project-local** — `installSkills` and `installRules` were called with `HOME` instead of `projectRoot`, causing skills and rules to land in `~/.claude/` instead of `<project>/.claude/`. Matches `install.sh` behavior and PEDI's verified working structure. Extends `test-installer-parity.sh` with 3 new checks (f): project-local `rules/`, project-local `skills/`, and a guard that `HOME` is not leaked. Parity harness: 8/8 checks green.
- **W7-5a — inject-settings-hooks.js tolerates UTF-8 BOM in settings.json** — `parseJsonSafe` in `_helpers/json-safe.mjs` now strips a leading U+FEFF BOM before parsing. Windows editors (Notepad, some PowerShell write patterns) sometimes prefix JSON files with the 3-byte BOM sequence `EF BB BF`; `JSON.parse` rejects this with "Unexpected token '﻿'". Fix is in the shared helper so both the main `mergeIntoFile` path and the `--dry-run` path benefit. Written output never contains BOM. Adds 5 tests in `test/json-safe.test.mjs`.
- **W7-5b — lib/install.js reports hook injection failure honestly** — `_injectHooks()` now returns a boolean; `run()` gates `✓ Hooks registered` on the return value. A non-zero exit from `inject-settings-hooks.js` (e.g. BOM-malformed settings.json) now prints a warning with a remediation hint instead of a false success tick. Adds 4 unit tests in `installer-platform.test.js`.
- **W7-3 — extension creates named pipe on Windows** — `server.ts` was always calling `path.join(workspaceRoot, socketRel)`, producing a Unix socket path (`C:\…\.claws\claws.sock`) instead of a Windows named pipe. `transport.ts` existed with the correct `getServerEndpoint()` helper but was never imported by `server.ts`. Fix: import `getServerEndpoint` + `isNamedPipe` from `transport.ts`; on `win32` `start()` uses `getServerEndpoint()` and skips `fs.mkdirSync`; `prepareSocket()` probes the pipe directly instead of calling `fs.existsSync`; `bind()` skips `process.umask`/`fs.chmodSync`; `stop()` skips `fs.unlinkSync` (named pipes are reclaimed by the kernel when the last handle closes).
- **W7-2b — inject-settings-hooks.js removes orphan hooks from pre-fix installs** — `removeOrphanHooks()` now runs before the existing `_source`-based dedup in all four code paths (add, `--update`, `--remove`, `--dry-run`). It removes entries that lack a `_source` field entirely and whose command matches a Claws hook script filename pattern (`session-start-claws.js`, `pre-tool-use-claws.js`, etc.) or contains `claws-install-` (the Windows temp-dir install pattern from `install.ps1`). Conservative: only entries with no `_source` at all are touched; hooks from third-party tools (any non-empty `_source`) are left alone. Adds 4 new tests to `inject-settings-dedup.test.js`.
- **W7-2 — hooks point at project-local .claws-bin, not temp extract dir** — `_injectHooks()` now receives `projectRoot` and passes `projectRoot/.claws-bin` to `inject-settings-hooks.js` as `CLAWS_BIN`. Previously it passed `REPO_ROOT/scripts`, which on Windows `iwr|iex` installs is the temp extract dir that `install.ps1` deletes after install — every hook invocation failed with `node:internal/modules/cjs/loader` ENOENT. `hookCmd()` in inject-settings-hooks.js already builds `CLAWS_BIN/hooks/<script>.js`; the fix makes that resolve to the persistent project-local path that `_prepareCLawsBin()` populates.
- **W7-1 — lib/install.js npx --yes for @vscode/vsce** — `spawnSync('npx', ...)` call that packages the VSIX now passes `--yes` as the first npx argument. Without it, npx prompts "Ok to proceed?" when `@vscode/vsce` isn't cached; fresh Windows VMs running via Azure Run Command have no stdin, so the install hung forever at phase 7.
...
### Internal
- **v0.8 phase 7 — lib/install.js owns extension build+install (cross-platform)** — `_installExtension` rewritten to own the full pipeline when `codeCli` is present: `npm install` (no `--omit=optional` — node-pty required by bundle-native.mjs) → `npm run build` → `npx @vscode/vsce package --no-dependencies -o REPO_ROOT/claws-code-<version>.vsix` → `code --install-extension` → verify via `--list-extensions`. All `spawnSync` calls use `shell: isWin` for npm/npx/code resolution on Windows; win32 npm-install failure prints actionable `winget install Microsoft.VisualStudio.BuildTools` guidance and skips extension install rather than aborting the whole installer. `_printSuccess` now uses `process.platform === 'darwin' ? 'Cmd+Shift+P' : 'Ctrl+Shift+P'` instead of hardcoded `Cmd+Shift+P`. Parity harness 5/5, full extension suite 0 failures.
- **v0.8 P4a — Windows dogfood scriptable matrix (16 items) + bug fixes** — Full Part 4a run on Windows Server 2022 VM (claws-win-vm, Standard_D2s_v3, Azure Run Command). VSIX built (1MB, 43 files). Installer ran cleanly: 8/8 phases, shell hook no-op on win32 confirmed. Matrix: 10 PASS / 5 SKIP (require interactive VS Code GUI) / 1 FAIL-documentation (Item 24: claws-fix.md lacks named-pipe guidance). Confirmed: extension installs (`neunaha.claws`), re-install sweeps stale commands (Bug 1), no Defender threats, OneDrive + long-path warnings fire, mcp_server.js has win32 named-pipe support (`CLAWS_SOCKET` ref + 7 win32/pipe refs), uninstall removes `.claws-bin` + CLAUDE.md block + hooks. VM deallocated. Full results in `.local/audits/p4a-dogfood-report.md`. Three runtime bugs fixed: reparse-point guard in `_prepareCLawsBin`, win32 shell hook no-op, project-local command path in `verify()`.
- **v0.8 P4a bug fixes — reparse-point guard + win32 shell hook no-op + project-local verify** — Three Windows-specific bugs surfaced during P3.4 dogfood. (1) `lib/install.js _prepareCLawsBin()`: `fs.rmSync(.claws-bin, {recursive, force})` before `mkdirSync` clears reparse-point artifacts left by macOS tarballs extracted on Windows (was causing EPERM on `mkdir hooks`); EEXIST after cleanup is caught and logged rather than throwing. (2) `lib/shell-hook.js injectShellHook()`: early-return with `"shell hook no-op on win32"` log on `process.platform === 'win32'` — prevents bash syntax (`[ -f ... ] && source ...`) being injected into PowerShell profiles, which broke PS startup silently. (3) `lib/verify.js verify()`: check `<projectRoot>/.claude/commands/claws.md` first; fall back to global `~/.claude/commands/` only when `projectRoot` is not given — installer installs commands project-locally so the old global-only check always emitted a false warning on first install. All changes: parity harness still green (5/5), extension suite 0 failures.
- **v0.8 P3.4 — installer parity harness + Windows VM smoke install** — `scripts/test-installer-parity.sh`: 5-check parity harness (bash `install.sh` vs Node `bin/cli.js install`) across `.mcp.json`, `CLAUDE.md CLAWS:BEGIN` block, `.claude/settings.json` hooks, `.claws-bin/` file list, and shell rc-file. All 5 checks green on macOS 14. `lib/install.js`: four fixes surfaced by the harness — project-local command target (`installCommands(projectRoot)`, was `HOME`), phase reordering (commands phase 3 before CLAUDE.md phase 5), `_writeInstallCommand()` writes `claws-install.md` with `npx claws-code install` content, `_prepareCLawsBin()` now copies `shell-hook.sh` + `schemas/` + `claws-sdk.js` + `README.md`. Windows VM smoke: all 8 installer phases completed on Windows Server 2022 (claws-win-vm, Standard_D2s_v3, westus2) via `node bin/cli.js install --no-hooks`; artifacts validated (mcp_server.js, stream-events.js, monitor-arm-watch.js, package.json, CLAUDE.md CLAWS:BEGIN, package.json name). VM deallocated. Notes for P4: macOS reparse-point artifacts in tar require `.claws-bin` pre-removal; shell hook writes bash syntax into PS profile (guard needed for Windows). See `.local/audits/p3-4-smoke-report.md`.
