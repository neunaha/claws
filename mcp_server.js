...
async function runBlockingWorker(sock, args) {
  ...
  const _bMonitorCmd = `Monitor(command="CLAWS_TOPIC='**' CLAWS_PEER_NAME='monitor-term-${termId}' CLAWS_ROLE='observer' node ${STREAM_EVENTS_JS} 2>&1 | grep --line-buffered '\\"correlation_id\\":\\"${_bCorrId}\\"' | awk '/system\\\\.(worker\\\\.completed|terminal\\\\.closed|worker\\\\.terminated)/{print; fflush(); exit}'", description="claws monitor | term=${termId} | corr=${_bCorrId.slice(0,8)} | sess=${new Date().toISOString().slice(0,13)}", timeout_ms=600000, persistent=false)`;
...
async function _dispatchTool(name, args, sock) {
  ...
  if (name === 'claws_create') {
    ...
    const _createMonitorCmd = `Monitor(command="CLAWS_TOPIC='**' CLAWS_PEER_NAME='monitor-term-${_createTermId}' CLAWS_ROLE='observer' node ${STREAM_EVENTS_JS} 2>&1 | grep --line-buffered '\\"correlation_id\\":\\"${_createCorrId}\\"' | awk '/system\\\\.(worker\\\\.completed|terminal\\\\.closed|worker\\\\.terminated)/{print; fflush(); exit}'", description="claws monitor | term=${_createTermId} | corr=${_createCorrId.slice(0,8)} | sess=${new Date().toISOString().slice(0,13)}", timeout_ms=600000, persistent=false)`;
  ...
  if (name === 'claws_worker') {
    ...
    const _fpMonitorCmd = `Monitor(command="CLAWS_TOPIC='**' CLAWS_PEER_NAME='monitor-term-${termId}' CLAWS_ROLE='observer' node ${STREAM_EVENTS_JS} 2>&1 | grep --line-buffered '\\"correlation_id\\":\\"${_fpCorrId}\\"' | awk '/system\\\\.(worker\\\\.completed|terminal\\\\.closed|worker\\\\.terminated)/{print; fflush(); exit}'", description="claws monitor | term=${termId} | corr=${_fpCorrId.slice(0,8)} | sess=${new Date().toISOString().slice(0,13)}", timeout_ms=600000, persistent=false)`;
        ...
        workers: results.map((r, i) => {
          ...
          return {
            ...
            monitor_arm_command: (_fTermId && _fCorrId)
              ? `Monitor(command="CLAWS_TOPIC='**' CLAWS_PEER_NAME='monitor-term-${_fTermId}' CLAWS_ROLE='observer' node ${STREAM_EVENTS_JS} 2>&1 | grep --line-buffered '\\"correlation_id\\":\\"${_fCorrId}\\"' | awk '/system\\\\.(worker\\\\.completed|terminal\\\\.closed|worker\\\\.terminated)/{print; fflush(); exit}'", description="claws monitor | term=${_fTermId} | corr=${_fCorrId.slice(0,8)} | sess=${new Date().toISOString().slice(0,13)}", timeout_ms=600000, persistent=false)`
              : null,
          };
        }),
        monitor_arm_required: true,
  ...
  if (name === 'claws_dispatch_subworker') {
    ...
    const _dswMonitorCmd = `Monitor(command="CLAWS_TOPIC='**' CLAWS_PEER_NAME='monitor-term-${termId}' CLAWS_ROLE='observer' node ${STREAM_EVENTS_JS} 2>&1 | grep --line-buffered '\\"correlation_id\\":\\"${_dswCorrId}\\"' | awk '/system\\\\.(worker\\\\.completed|terminal\\\\.closed|worker\\\\.terminated)/{print; fflush(); exit}'", description="claws monitor | term=${termId} | corr=${_dswCorrId.slice(0,8)} | sess=${new Date().toISOString().slice(0,13)}", timeout_ms=600000, persistent=false)`;
